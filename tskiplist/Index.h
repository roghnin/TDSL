...
{
public:
    IndexOperation(Node<K,V> * node, OperationType op) : node(node), op(op) {}
...
{
public:
    Index(unsigned int version) : head(K(), version, MIN) {
        skiplist_init(&sl, NodeCmp<K,V>);
        skiplist_insert(&sl, &head.snode);
    }

    void update(std::vector<IndexOperation<K,V>> & ops){
        for (auto & op : ops) {
